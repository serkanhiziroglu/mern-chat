...
export default function Chat() {
    ...
    return (
        <div className="flex h-screen">
            <div className="bg-blue-100 w-1/3 p-4 mb-4 justify-center items-center">
                ...
                <div className="justify-center items-center">
                    {Object.keys(onlinePeopleExclude).map(userId => (
                        <div key={userId} onClick={() => setSelectedUserId(userId)} className={"border-b border-gray-300 py-2 flex cursor-pointer " + (userId === selectedUserId ? 'bg-blue-50' : "")}>
                            <div className="mt-2 mr-2" style={{ width: '10px', height: '10px', borderRadius: '50%', backgroundColor: '#A3EBB1' }}></div>
                            <span>{onlinePeopleExclude[userId]}</span>
                        </div>
                    ))}
                </div>
            </div>
                        ...
                        <div className="overflow-y-scroll absolute top-0 left-0 right-0 bottom-2">
                            {messagesWithoutDupes.map(message => (
                                <div key={message._id} className={message.sender === id ? 'text-right' : 'text-left'}>
                                    <div className={"text-left inline-block p-2 my-2 rounded-md text-sm " + (message.sender === id ? 'bg-blue-500 text-white self-start' : 'bg-white text-gray-500')}>
                                        {message.text}
                                    </div>
                                </div>
                            ))}
